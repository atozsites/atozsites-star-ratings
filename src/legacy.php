...
if (! function_exists('Atozsites_star_ratings')) {
    function Atozsites_star_ratings($idOrPost = null)
    {
        $id = null;
        ...
        return Atozsites\StarRating\response(compact('id'), false);
    }
}

if (! function_exists('Atozsites_star_ratings_get')) {
    function Atozsites_star_ratings_get($limit = 5, $taxonomyId = null, $offset = 0)
    {
        global $wpdb;
        $postsTable = $wpdb->posts;
        $postMetaTable = $wpdb->prefix.'postmeta';
        $base = get_option(Atozsites\StarRating\prefix('stars')) / 5;
        ...
        $queryConditions = "
            WHERE
                ...
                AND postmeta_count.meta_key = '_Atozsites_casts'
                AND postmeta_ratings.meta_key = '_Atozsites_ratings'
        ";
